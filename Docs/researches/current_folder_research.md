...
2. 多技术栈聚合
- Rust 主实现位于 `codex-rs/`，且是一个大型 workspace（`codex-rs/Cargo.toml:1-157`）。
- Node/PNPM 工作区承载 npm 分发封装、TypeScript SDK、shell-tool-mcp（`pnpm-workspace.yaml:1-12`）。
...
5. 发布产物覆盖多平台、多封装形式
- Rust release 工作流构建多 target、签名并打包（`.github/workflows/rust-release.yml:8-280`）。
...
8. 通过研究自动化持续覆盖仓库对象
- `.ops/research_guard.sh` 会读取 checklist 首个待办并下发标准化研究任务模板（`.ops/research_guard.sh:140-205`）。
...
1. 用户安装并执行 `codex`：
- npm 包入口 `codex-cli/bin/codex.js` 根据 `process.platform/process.arch` 解析目标 triple（`codex-cli/bin/codex.js:24-67`）。
- 解析对应可选依赖包，定位 `vendor/<triple>/codex/{codex|codex.exe}`（`codex-cli/bin/codex.js:73-119`）。
- 将 `vendor/<triple>/path` 注入 PATH（用于 bundled `rg` 等），`spawn` 原生二进制并转发信号（`codex-cli/bin/codex.js:161-229`）。
...
- `rust-ci.yml`：变更路径检测后按矩阵运行 format/lint/build/test/argument-comment-lint 等（`.github/workflows/rust-ci.yml:12-320`）。
- `rust-release.yml`：tag 校验、跨平台构建、签名、打包发布（`.github/workflows/rust-release.yml:19-280`）。
...
3. 守护执行：
- `research_guard.sh` 自动读取首个 pending 项，拼装中文任务模板并调用 `codex --yolo exec`（`.ops/research_guard.sh:140-212`）。
...
- `.github/workflows/rust-release.yml:19-280`
...
1. Rust：Cargo workspace + 多 crate 内部依赖（`codex-rs/Cargo.toml:86-157`）。
2. JS：Node + PNPM workspace（`package.json:21-25`，`pnpm-workspace.yaml:1-12`）。
3. Bazel：rules_rs + llvm + platform toolchain（`MODULE.bazel:3-64`）。

### 3) 配置面外部输入

1. 用户配置：`~/.codex/config.toml`（`docs/config.md:11-13`）。
2. 环境变量：`CODEX_CA_CERTIFICATE` / `SSL_CERT_FILE` / `CODEX_SQLITE_HOME`（`docs/config.md:35-59`）。
3. 跨进程/系统能力：sandbox（Seatbelt/Landlock/bwrap/Windows）与 shell wrapper 协议（`codex-rs/core/README.md:9-80`，`codex-rs/shell-escalation/README.md:6-16`）。
...
- 研究守护和任务模板中存在 `git add -A` 模式，若工作区有非目标变更，可能误纳入提交（见 `.ops/research_guard.sh:214-227` 的 checkpoint 逻辑）。

### 边界

1. 根目录主要是编排层，不是业务算法层。
2. 具体 agent 推理/执行策略在 `codex-rs/core` 与相关 crate，而非根脚本本身。
3. 文档中有大量跳转到官方站点，仓库内文档偏“索引式”。
