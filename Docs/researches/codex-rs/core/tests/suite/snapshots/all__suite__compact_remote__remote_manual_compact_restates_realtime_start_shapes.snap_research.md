# Research: remote_manual_compact_restates_realtime_start_shapes.snap

## 场景与职责

该快照文件记录了**远程手动压缩后重述实时开始指令（Remote Manual Compact Restates Realtime Start）**场景，验证当实时对话仍处于活动状态时执行手动压缩，后续回合如何重述实时开始指令。

**测试场景**：用户开启实时对话，发送消息，执行手动远程压缩（实时仍活动），然后发送新消息。
...
```
开启实时对话 → USER_ONE → 手动远程压缩（实时仍活动）→ USER_TWO
```
...
- **测试函数**: `snapshot_request_shape_remote_manual_compact_restates_realtime_start` (行 1675-1760)
- **快照生成**: 行 1743-1755
...
let compact_mock = responses::mount_compact_json_once(
    &server,
    serde_json::json!({
        "output": compacted_summary_only_output("REMOTE_MANUAL_REALTIME_STILL_ACTIVE_SUMMARY")
    }),
).await;
```
...
### 相关测试
- `remote_compact_resume_restates_realtime_end_shapes`: 恢复后重述结束指令
- `remote_pre_turn_compaction_restates_realtime_start_shapes`: 轮前压缩场景
