...
`frame_8.txt` 是 Codex TUI 中 `dots` 动画系列的第8帧（索引7），在36帧动画循环中代表约22.2%的时间点。该帧展示点状图案接近扩张峰值的状态。

## 功能点目的

- **扩张后期**：图案接近扩张峰值
- **峰值准备**：为扩张峰值（frame_9-10）做准备
- **节奏维持**：维持"呼吸"动画的节奏
...
codex-rs/tui/frames/dots/frame_8.txt
    ↓ include_str! 宏
codex-rs/tui/src/frames.rs: FRAMES_DOTS[7]
    ↓ 运行时访问
...
### 潜在风险
1. **扩张节奏**：作为扩张后期的帧，影响用户对动画节奏的感受
2. **过渡平滑性**：需要确保与前后帧的视觉连贯

### 边界情况
1. **快速操作**：如果操作在640ms内完成，用户可能只看到前8帧
2. **动画中断**：用户可能在中途中断操作
