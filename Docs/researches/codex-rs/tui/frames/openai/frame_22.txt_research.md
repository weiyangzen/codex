...
- 该帧在动画中的视觉作用
  - 展示 OpenAI 螺旋标识的紧凑状态
  - 图案宽度最小，字符高度集中
  - 螺旋线条形成紧密的环形，准备进入下一周期的展开
- 与前后帧的关系
  - 承接第21帧的接近紧凑状态，达到收缩终点
  - 为第23帧的重新展开做准备

## 具体技术实现
- 字符构成分析
  - 主要字符：`a`, `e`, `i`, `n`, `o`, `p`（小写字母）
  - 特征：第3行 `apeoieaeieniinipeopa` 和第4行 `pnoninooa    aoonoopaapp` 显示紧凑形态
  - 字符分布：宽度约28字符，字符高度集中
- 视觉图案描述
  - 顶部：`appnpnnpnppa`, `apeoieaeieniinipeopa` 形成上边缘
  - 中部：`niieo            paeinieenoi`, `nioea           pieinaea ninni` 显示大面积中心空白
  - 底部：`oioianaaapnaenoa` 形成下边缘
  - 整体呈紧凑的椭圆形态，环形结构最紧密
- 尺寸规格（17行 x ~40字符）
  - 实际宽度：约28字符的有效内容
  - 高度：17行（含上下空白边距）
...
- 编译时依赖：通过 `frames_for!("openai")` 宏嵌入
  - 需要 `frames.rs` 中的宏定义
  - 文件路径：`codex-rs/tui/frames/openai/frame_22.txt`
