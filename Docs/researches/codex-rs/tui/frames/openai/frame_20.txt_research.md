...
- 在动画序列中的位置（第20帧，共36帧）
  - 位于动画序列的中后段，承接第19帧，过渡到第21帧
  - 螺旋图案继续收缩，接近紧凑状态

## 功能点目的
- 该帧在动画中的视觉作用
  - 展示 OpenAI 螺旋标识的进一步收缩状态
  - 图案宽度明显减小，中心空白区域扩大
  - 螺旋线条更加集中，形成更清晰的环形轮廓
- 与前后帧的关系
  - 承接第19帧的收缩状态，继续向内收敛
...
- 字符构成分析
  ...
  - 特征：第3行 `pnnonpapnnniiaaonpp` 和第5行 `aioaoio           appineonn` 显示明显收缩
  - 字符分布：宽度约32字符，中心空白区域显著
- 视觉图案描述
  - 顶部：`aapniinpnpaa`, `pnnonpapnnniiaaonpp` 形成上边缘
  - 中部：`ai pio           ae aiiaoniin`, `eapio           poonioe  oniin` 显示大面积中心空白
  - 底部：`aopia naaaaiooo` 形成下边缘
  - 整体呈明显收缩的椭圆形态，中心空洞扩大
- 尺寸规格（17行 x ~40字符）
  - 实际宽度：约32字符的有效内容
  - 高度：17行（含上下空白边距）
...
- 编译时依赖：通过 `frames_for!("openai")` 宏嵌入
  - 需要 `frames.rs` 中的宏定义
  - 文件路径：`codex-rs/tui/frames/openai/frame_20.txt`
