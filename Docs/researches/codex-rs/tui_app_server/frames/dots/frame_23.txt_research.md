...
`frame_23.txt` 是 Codex TUI 应用服务器的 ASCII 艺术动画帧文件，属于 `dots` 动画变体系列中的第二十三帧。作为 36 帧循环动画序列的一部分，它继续推进点阵图案的动态演变，为用户提供持续的视觉反馈。

## 功能点目的

1. **动画序列延续**：作为第 23 帧，维持动画的视觉流动性
...
```rust
// 直接数组访问
let frame_23: &str = FRAMES_DOTS[22];

// 通过动画控制器（推荐）
let animation = AsciiAnimation::new(request_frame);
// 在 t ≈ 1800ms 时
let current = animation.current_frame(); // 返回 frame_23
```

### 渲染流程

```
FRAMES_DOTS[22] (&'static str)
    ↓
