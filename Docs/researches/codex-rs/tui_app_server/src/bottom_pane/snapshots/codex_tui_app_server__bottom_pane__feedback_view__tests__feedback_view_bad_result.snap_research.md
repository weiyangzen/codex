# Feedback View - Bad Result 快照研究

## 场景与职责

该快照展示了 **FeedbackNoteView** 组件在 **BadResult（结果不佳）** 反馈类别下的 UI 渲染效果。这是 Codex TUI 应用服务器中用户反馈收集流程的一部分。

**使用场景：**
- 用户对 AI 生成的结果不满意（输出偏离目标、不正确、不完整或无帮助）
- 用户通过快捷键或菜单触发反馈功能
- 系统显示反馈类别选择弹窗后，用户选择 "bad result" 类别
- 进入此视图收集用户的详细反馈描述

**核心职责：**
- 提供简洁的文本输入界面，收集用户对不佳结果的补充说明
- 显示类别特定的标题和占位提示文本
...
**渲染结构（从快照反推）：**
```
▌ Tell us more (bad result)     <- 标题行（青色分隔符 + 加粗标题）
▌                               <- 空行/输入区域顶部
...
- `BadResult` 类别对应标题："Tell us more (bad result)"
...
**测试代码位置：**
- 测试函数：`feedback_view_bad_result()` (行 642-647)
- 使用 `make_view(FeedbackCategory::BadResult)` 创建测试视图
- 渲染宽度：60 字符
...
2. **字符计数**
   - 可添加字符/字数统计，帮助用户控制描述长度

3. **富文本支持**
   - 当前仅支持纯文本，可考虑支持简单的 markdown 格式

4. **反馈模板**
   - 针对不同类别提供预设的反馈模板或问题引导
   - 例如 BadResult 可询问："预期结果是什么？实际结果是什么？"
